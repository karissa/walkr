...
public class GUI {
	...
	protected ImageIcon flagIcon = new ImageIcon("images/flag-black.png");
	...
	private Icon currentTrackerIcon = cameraIcon;
	...
	private void initialize() {
		noPicture = new ImageIcon("images/no-photo.gif");
		...
		frame.setTitle("Walkr: Fetch a Walking Tour of Campus");
		...
		/*
		  JEditorPane url = null;
		 
		try {
			url = new JEditorPane("http://www.iub.edu/~registra/buildingcodes.shtml");
		} catch (IOException e2) {
			// TODO Auto-generated catch block
			e2.printStackTrace();
		}
		url.setText("Building Codes");
		url.setBounds(286+148+50, 7,148,29);
		url.setOpaque(false);
		...
		url.addHyperlinkListener(new HyperlinkListener() {
			...
			public void hyperlinkUpdate(HyperlinkEvent evt) {
				...
				try { // Show the new page in the editor pane. 
	...
	protected void setSkin(String color) {
		...
		if(skin.equals("crimson")){
			Color crimson = new Color(125,26,6);
			frame.getContentPane().setBackground(crimson);
			lblFrom.setForeground(Color.WHITE);
			lblToward.setForeground(Color.WHITE);
			list.setBackground(crimson);
		}
	}
/**
 * 
 ...
 */
	...
	class PreferencesPanel extends JDialog {
		...
		public PreferencesPanel() {
			...
			comboBox_skin.setModel(new DefaultComboBoxModel(new String[] {"Yellow", "Green", "Black", "Crimson"}));
						...
						public void actionPerformed(ActionEvent arg0) {
							...
							if(enableTracker.isSelected()){
								if(walkr.hasPath()) {
									...
									trackerEnabled = true;
									enableTracker.setSelected(true);
								} else {
